...
async function runMistral(imagePath) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Mistral OCR failed with ${response.status}: ${errorText}`);
    }
...
async function runGoogleVision(imagePath) {
    const actualPath = await getImagePath(imagePath);
    const image = await import("node:fs/promises").then((fs) => fs.readFile(actualPath, { encoding: "base64" }));
    const response = await axios.post(`https://vision.googleapis.com/v1/images:annotate?key=${env.googleVisionApiKey}`, {
        requests: [
            {
                image: { content: image },
...
async function runAzureVision(imagePath) {
    const actualPath = await getImagePath(imagePath);
    const image = await import("node:fs/promises").then((fs) => fs.readFile(actualPath));
    const response = await axios.post(`${env.azureVisionEndpoint}/vision/v3.2/read/analyze`, image, {
        headers: {
...
export async function extractTextFromCard(imagePath) {
    try {
