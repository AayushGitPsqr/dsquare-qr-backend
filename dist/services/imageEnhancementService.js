 ...
 * - Optimizes quality
 */
export async function enhanceImageForOCR(imagePath, config = {}) {
    try {
        ...
        const enhancedPath = imagePath.replace(ext, `-enhanced${ext}`);
        ...
        logWarn(`Failed to enhance image ${imagePath}: ${error instanceof Error ? error.message : "Unknown error"}`);
 ...
 */
export async function enhanceMultipleImages(imagePaths, config) {
    return Promise.all(imagePaths.map((path) => enhanceImageForOCR(path, config)));
}
