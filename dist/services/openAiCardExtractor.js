...
export async function extractBusinessCardWithOpenAI(imagePath) {
    if (!env.llmApiKey) {
        return null;
    }
    ...
    const response = await fetch(env.llmApiUrl, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "Read the business card image directly and extract the fields. Be conservative. Use null for any field you cannot confidently identify. Infer unlabeled name, company, designation, address, email, phone, and website only when strongly supported by the visual layout or text. Do not invent values."
                },
                {
                    role: "user",
                    content: [
                        {
                            type: "text",
                            text: "Extract business-card data from this image and return only the JSON object matching the schema."
                        },
                        {
                            type: "image_url",
                            image_url: {
