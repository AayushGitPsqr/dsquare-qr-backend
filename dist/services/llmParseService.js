...
export function normalizeLlMParsedFields(payload) {
...
export async function parseBusinessCardWithLLM(rawText) {
    if (!env.llmApiKey) {
        return null;
    }
    const response = await fetch(env.llmApiUrl, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: "system",
                    content: "You extract business card contact fields from OCR text. Return only JSON with keys: name, designation, company, email, phone, website, address. Use null for unknown values. Do not invent data. If a value is present without a label, infer the most likely field. Do not include any other keys."
                },
                {
                    role: "user",
                    content: `OCR text:\n${rawText}`
