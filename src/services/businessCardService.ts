...
import {
  ...
  getBusinessCardById,
  listBusinessCards
} from "../repositories/businessCardRepository.js";
...
export async function scanBusinessCard(imagePath: string) {
      ...
      if (openAiParsed) {
        ...
        if (!valueOrEmpty(normalized.name)) {
          const error = new Error(
            "OpenAI could not confidently identify the card owner name."
          );
          error.name = "OcrError";
          throw error;
        }
...
export function validateBusinessCardPayload(payload: Partial<BusinessCardDocument>) {
  const errors: string[] = [];
...
export async function saveBusinessCard(payload: Partial<BusinessCardDocument>) {
