...
export function validateUpdateClientRequestStatus(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (
    !(CLIENT_REQUEST_STATUSES as readonly string[]).includes(String(status))
  ) {
    return sendError(
      res,
  ...
  req.body.status = status as ClientRequestStatus;
  return next();
}
